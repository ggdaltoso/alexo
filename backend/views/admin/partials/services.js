 ...
 * dentro do #services. O bloco se repinta sozinho a cada 2s e depois de cada
...
const DOT_BY_STATE = { active: 'dot-ok', failed: 'dot-error', inactive: 'dot-off' };
...
function buttons(s) {
  ...
  const common = (action) =>
    `hx-post="${route(action)}" hx-target="#services" hx-swap="innerHTML"` +
    ` hx-disabled-elt="#services button"`;
...
module.exports = function servicesBlock(services) {
  return services.map((s) => {
    ...
    return `<div class="row">` +
      `<span class="key-cell">` +
        `<span class="dot ${DOT_BY_STATE[s.state] || 'dot-mid'}" title="${title}"></span>` +
        `${esc(s.label)}<br><small>${esc(s.description)}</small>` +
      `</span>` +
      `<span class="buttons">${buttons(s)}</span>` +
    `</div>`;
  }).join('');
};
