  ...
  return (
    <Frame className="flex items-center gap-2">
      ...
      <div className="flex-1">
        <div className="text-[0.35rem] font-bold">{currency}</div>
        <div className="flex flex-col gap-1 ">
          <span className="text-[0.75rem] font-bold">R$ {bid.toFixed(2)}</span>
          <div
          ...
          >
            {isPositive ? (
              <Mailnews17 className="size-2" variant="16x16_4" />
            ) : (
              <Mailnews18 className="size-2" variant="16x16_4" />
            )}
            <span>
              {isPositive ? '+' : ''}
              {pctChange.toFixed(2)}%
...
export const ExchangeRateScreen = React.memo(() => {
  ...
  return (
    <Frame
      boxShadow="$out"
      bgColor="$material"
      className="p-2 h-full flex flex-col gap-2 justify-evenly"
    >
      <CurrencyCard
        icon={<Printer className="size-6" variant="32x32_4" />}
      ...
      <CurrencyCard
        icon={<Progman24 className="size-6" variant="32x32_4" />}
