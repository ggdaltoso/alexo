...
### Planned: NFC Music Player

Not implemented yet — hardware/software design is fully documented in [`docs/nfc-music-player-plan.md`](docs/nfc-music-player-plan.md) on the `feature/nfc-music-player` branch. Physical wiring and data flow once built:

```mermaid
flowchart TB
    subgraph hw["Physical components"]
    ...
    subgraph sw["Backend (Node.js, running on the Pi)"]
    ...
    subgraph fe["Frontend (React, same device)"]
